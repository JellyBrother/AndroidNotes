...
public class RoundedDrawable extends Drawable {
    ...
    private float mRoundImgPx = 0;
    private ColorStateList mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
    private ScaleType mScaleType = ScaleType.FIT_CENTER;

    public RoundedDrawable(Bitmap bitmap) {
        if (mRoundImgPx == 0f) {
            mRoundImgPx = BaseCommon.Base.application.getResources().getDimension(R.dimen.base_dimen_36dp);
        }
        float scaleX = mRoundImgPx / (float) bitmap.getWidth();
        float scaleY = mRoundImgPx / (float) bitmap.getHeight();
        bitmap = Bitmap.createScaledBitmap(bitmap, (int) (bitmap.getWidth() * scaleX), (int) (bitmap.getHeight() * scaleY), true);
        mBitmap = bitmap;
...
//        }
    ...
    public static Bitmap drawableToBitmap(Drawable drawable) {
        ...
        try {
            ...
            LogUtil.getInstance().e(TAG, "Failed to create bitmap from drawable!" + e.toString());
            bitmap = null;
        }
