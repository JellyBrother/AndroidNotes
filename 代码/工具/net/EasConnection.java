...
public class EasServerConnection {
    ...
    public static final int STOPPED_REASON_CANCEL = 3;

    private static final String[] ACCOUNT_SECURITY_KEY_PROJECTION =
            { *Content.AccountColumns.SECURITY_SYNC_KEY };
    ...
    public void redirectHostAuth(final String newAddress) {
        ...
        if (mHostAuth.isSaved()) {
            ...
            cv.put(*Content.HostAuthColumns.ADDRESS, newAddress);
    ...
    private String makeBaseUriString() {
        return *ClientConnectionManager.makeScheme(mHostAuth.shouldUseSsl(),
    ...
    public HttpPost makePost(final String uri, final HttpEntity entity, final String contentType,
        ...
        if (usePolicyKey) {
            ...
            if (mAccountId == Account.NO_ACCOUNT) {
                ...
                accountKey = ContentExs.accessFirstRow(mContext.getContentResolver(),
                        ContentUris.withAppendedId(Account.CONTENT_URI, mAccountId),
                        ACCOUNT_SECURITY_KEY_PROJECTION, null, null, null)
                        .getString(0, null);
            }
    ...
    public EasResponse executeHttpUriRequest(final HttpUriRequest method, final long timeout)
        ...
        LogUtils.d(TAG, "EasServerConnection about to make request");
        ...
        try {
            ...
            if(time > 3000) {
                // 请求耗时超过3秒的打印调用堆栈
                StackTraceDumper.dumpCurrentStackTraces("executeHttpUriRequest cost much time " + time );
            }
        }
    }
    ...
    public synchronized void stop(final int reason) {
        ...
        if (reason >= STOPPED_REASON_ABORT && reason <= STOPPED_REASON_CANCEL) {
            final boolean isMidPost = (mPendingRequest != null);
            LogUtils.i(TAG, "%s with reason %d", (isMidPost ? "Interrupt" : "Stop next"), reason);
            mStoppedReason = reason;
            if (isMidPost) {
                SimpleExecutorJvm.execute(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            mPendingRequest.abort();
                        } catch (Exception e) {
                            LogUtils.e(TAG, "Some thing wrong when abort a request: " + e);
                        }
                    }
                }).joinClose();
            } else {
