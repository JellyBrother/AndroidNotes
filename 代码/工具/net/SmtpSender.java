...
public class SmtpSender extends Sender {
    ...
    public SmtpSender(Context context, Account account) {
        ...
        mTransport = new *Transport(context, "SMTP", sendAuth);
     ...
     */
    public void setTransport(*Transport testTransport) {
    ...
    public void open() throws MessagingException {
        try {
             ...
             * using HELO on non STARTTLS connections because of AOL's *
            ...
            LogUtils.e(Logging.LOG_TAG, e, "open SSLException");
            throw new CertificateValidationException(e.getMessage(), e);
        } catch (IOException ioe) {
            LogUtils.e(Logging.LOG_TAG, ioe, "open IOException");
    ...
    public void sendMessage(long messageId) throws MessagingException {
        ...
        try {
            try {
                executeSimpleCommand("* FROM:" + "<" + from.getAddress() + ">");
            } catch (Exception e){
                LogUtils.e(LogUtils.TAG, e, "sendmessage failed");
                if (!from.getAddress().equals(mUsername)) {
                    from.setAddress(mUsername);
                } else if (!from.getAddress().contains("@")) {
                    from.setAddress(from.getAddress() + "@" +
                            StringEx.toServerName(mAccount.mHostAuthRecv.mAddress).getDomain());
                } else {
                    from.setAddress(StringEx.to*Name(from.getAddress()).getUserName()) ;
                }
                message.mFrom = from.toHeader();
                executeSimpleCommand("* FROM:" + "<" + from.getAddress() + ">");
            }
            for (Address address : to) {
                String addressString = address.getAddress().trim();
                executeSimpleCommand("RCPT TO:" + "<" + addressString + ">");
            ...
            AppDataManager.getContactDelegate().updateContact2ChatTime(addressList);
        } catch (IOException ioe) {
    ...
    @Override
    public void sendUnloadMessage(long messageId, List<*Content.Attachment> attachments) throws MessagingException {
        close();
        open();
        ...
        try {
            ...
            Rfc822Output.writeTo(mContext, message,
                    ...
                    false /* do not use smart reply */,
                    false /* do not send BCC */,
                    attachments  /* attachments are in the message itself */);
            executeSimpleCommand("\r\n.");
            AppDataManager.getContactDelegate().updateContact2ChatTime(addressList);
        } catch (IOException ioe) {
            throw new MessagingException("Unable to send message", ioe);
